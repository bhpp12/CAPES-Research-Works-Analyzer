...
def carregar_dataset(caminho):
    ...
    for _, row in df.iterrows():
        trabalho = Trabalho(
            (row.get("NM_PROGRAMA") or "").strip(), # Programa do trabalho
            (row.get("NM_ORIENTADOR") or "").strip(), # Nome do orientador
            (row.get("NM_GRANDE_AREA_CONHECIMENTO") or "").strip(), # Grande área
            (row.get("NM_AREA_CONHECIMENTO") or "").strip(), # Área específica
            (row.get("NM_PRODUCAO") or "").strip(), # Título do trabalho
        )
