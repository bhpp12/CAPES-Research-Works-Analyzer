...
def dividir_lista(lista_completa, quantidade_blocos):
    ...
    tamanho_base = tamanho_total // quantidade_blocos # Determina o tamanho base de cada bloco usando divisão inteira
    sobra = tamanho_total % quantidade_blocos    # Calcula o restante (sobra) que não foi distribuído na divisão

    blocos = []    # Lista que irá armazenar os blocos divididos
    inicio = 0     # Índice de início de cada bloco na lista original

    for i in range(quantidade_blocos):
        if i < sobra:
            resto = 1  # Adiciona um elemento extra a este bloco
        else:
            resto = 0  # Sem elemento extra neste bloco
        fim = inicio + tamanho_base + resto  # Define o índice de fim do bloco atual, somando o tamanho base e o resto


        blocos.append(lista_completa[inicio:fim])  # Extrai a sublista do índice 'inicio' até 'fim' e adiciona à lista de blocos
        inicio = fim    # Atualiza o índice de início para o próximo bloco
